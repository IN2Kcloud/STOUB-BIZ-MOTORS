...
document.querySelectorAll(".content-inner p").forEach((p) => {
  ...
  wrapper.classList.add("content-marquee");

  const inner = document.createElement("div");
  inner.classList.add("content-marquee-inner");
...
document.querySelectorAll(".content-marquee-inner").forEach((el) => {
...
document.querySelectorAll(".footer-inner p").forEach((p) => {
  ...
  const inner = document.createElement("div");
  inner.classList.add("footer-marquee-inner");
...
document.querySelectorAll(".footer-marquee-inner").forEach((el) => {
  const width = el.scrollWidth / 2;
...
function draw() {
    ...
    const baseRadius = Math.max(gridCanvas.width, gridCanvas.height) * .9;
    const pulseRadius = baseRadius + Math.sin(time * 0.5) * 100;

    const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, pulseRadius);
    gradient.addColorStop(0, "#6ABD45"); 
